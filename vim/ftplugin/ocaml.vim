...
class Annotations:
    ...
    def __init__(self):
        ...
        self.__re_int_ref = re.compile('^int_ref\s+(\S+)\s"[^"]*"\s+(\d+)\s+(\d+)\s+(\d+)')
    ...
    def parse(self):
        try:
            ...
            try:
              ...
              annot_file = os.path.join(head2, "_build", tail2, tail)
              self.__parse(annot_file)
...
function OCamlBuild(prog)
  let old_makeprg = &makeprg
  exec ':set makeprg=ocamlbuild\ '.a:prog
  exec ':make'
  exec ':set makeprg='.old_makeprg
endfunction
